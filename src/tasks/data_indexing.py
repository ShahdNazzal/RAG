        ...
        while has_records:
            ...
            if not page_chunks or len(page_chunks) == 0:
                ...
                break
                            ...
                            # فلترة الـ chunks الفاضية أو القصيرة جداً قبل الإرسال لـ Gemini
            valid_chunks = [
                c for c in page_chunks
                if c.chunk_text and c.chunk_text.strip()
            ]
            skipped_count = len(page_chunks) - len(valid_chunks)
            if skipped_count > 0:
                logger.warning(f"Skipped {skipped_count} empty chunk(s) for project_id: {project_id}")
            ...
            if not valid_chunks:
                pbar.update(len(page_chunks))
                continue
            chunks_ids = [c.chunk_id for c in valid_chunks]
